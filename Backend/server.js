...
let carts = {}; // cartId -> cart items array
let orders = [];
...
function getCartWithTotals(cartId = GUEST_CART_ID) {
...
app.post('/api/cart/add', (req, res) => {
  ...
  const existingItem = carts[cartId].find(item => item.id === productId);
  if (existingItem) {
    existingItem.quantity += quantity;
  } else {
    carts[cartId].push({ ...product, quantity });
  }
  
  const cart = getCartWithTotals(cartId);
  res.status(201).json({ success: true, message: 'Item added to cart', cart });
});

app.get('/api/cart', (req, res) => {
  const cart = getCartWithTotals(GUEST_CART_ID);
...
app.put('/api/cart/update', (req, res) => {
  ...
  const item = carts[cartId].find(item => item.id === productId);
  if (item) {
    item.quantity = quantity;
    if (!Number.isInteger(item.quantity) || item.quantity <= 0) {
      carts[cartId] = carts[cartId].filter(i => i.id !== productId);
    }
  }
...
app.delete('/api/cart/remove/:productId', (req, res) => {
  ...
  carts[cartId] = (carts[cartId] || []).filter(item => item.id !== Number.parseInt(req.params.productId, 10));
  const cart = getCartWithTotals(cartId);
...
app.post('/api/orders/create', (req, res) => {
  ...
  orders.push(order);
  carts[cartId] = []; // Clear cart after order
